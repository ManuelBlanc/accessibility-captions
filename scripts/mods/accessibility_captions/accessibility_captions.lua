...
function mod.on_all_mods_loaded()
    mod:hook("VMFOptionsView", "callback_setting_changed", function(func, self, mod_name, setting_id, old_value, new_value)
        if mod_name == "accessibility_captions" and new_value == true and find_by_label(setting_id) then
...
function mod.on_setting_changed(setting_id)
    ...
    if data then
        ...
        if data.enabled then
            play_sample_sound(data[math.random(#data)], max_duration)
        end
    else
...
local function trigger_event(data, pos)
    ...
    local t_start = Application.time_since_launch()
...
----------------------------------------------------------------------------------------------------
local function DEBUG_SOUND_SOURCE(f, e) return mod:debug("%s(%q)", f, e) end --IF_LINE DEBUG
